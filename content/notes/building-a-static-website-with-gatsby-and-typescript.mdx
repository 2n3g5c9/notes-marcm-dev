...
<BlockWarning>
  ...
  develop my understanding. Feel free to drop me a message if you want to share some advice.
</BlockWarning>
...
* Code quality checks: detects bugs, code duplication and code smells.
...
**Automated checks:**
* `.github/workflows/ci.yml` is where automated checks on push and pull requests are defined (via NPM scripts).
`node_modules` is cached with `actions/cache` and a [ZSTD](https://facebook.github.io/zstd/) compressed archive is used
as shared artifact for subsequent tests.
...
* [SonarCloud](https://sonarcloud.io/) ensures the code complies with high standards by passing the quality gate.
* `.github/workflows/codeql-analysis.yml` is where automated code vulnerability scanning is defined, leveraging
[CodeQL](https://codeql.github.com/), which is known as the project behind [LGTM](https://lgtm.com/).
* `lighthouserc.js` is where expectations on the automated [Lighthouse](https://github.com/GoogleChrome/lighthouse-ci)
audits are defined.
...
Achieving all perfect scores in Lighthouse (Chrome 90) wasn't that hard with Gatsby & Vercel. But it certainly became
...
score negatively. Also, [Google Tag Manager](https://tagmanager.google.com/) has a significant impact on performance.
